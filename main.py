...
def parse_rate(text: str) -> float | None:
    ...
    if not match:
        logging.error(f"[PARSE ERROR] No numeric value found in: {text}")
        return None
    raw = match.group(1)
    ...
    try:
        ...
        logging.error(f"[PARSE ERROR] Could not convert '{raw}' -> {ve}")
        return None
...
# --- Manual stealth setup ---
async def new_stealth_page(p, headless=True):
...
async def fetch_moneygram_rate(from_currency: str, to_currency: str) -> float | None:
    ...
    try:
        async with async_playwright() as p:
            browser, page = await new_stealth_page(p, headless=False)
...
async def fetch_wu_rate(from_currency: str, to_currency: str) -> float | None:
    ...
    try:
        async with async_playwright() as p:
            browser, page = await new_stealth_page(p, headless=True)
            await page.goto(config["url"], wait_until="domcontentloaded", timeout=60000)
            await page.wait_for_selector(config["selector"], timeout=60000)
            text = await page.locator(config["selector"]).inner_text()
            rate = parse_rate(text)
            if rate is not None:
                logging.info(f"[WU RATE ADDED] {from_currency}->{to_currency}: {rate}")
            else:
                logging.error(f"[WU PARSE ERROR] Could not extract number from: {text}")
            await browser.close()
            logging.info("[Browser closed]")
            return rate
    except Exception as e:
